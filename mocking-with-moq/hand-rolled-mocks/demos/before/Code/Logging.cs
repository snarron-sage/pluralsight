...
namespace PluralSight.HandRolledMocks.Code
{
    public class Logging
    {
        ...
        public Logging(IScrubSensitiveData scrubSensitiveData, ICreateLogEntryHeaders createLogEntryHeaders,
                       ICreateLogEntryFooter createLogEntryFooter, IConfigureSystem systemConfiguration)
        {
        ...
        public void CreateEntryFor(string message, LogLevel logLevel)
        {
            ...
            {
                Console.WriteLine(string.Format("Stack /n/n {0}", Environment.StackTrace));
            }

            Console.WriteLine(string.Format("{0} - {1}", logLevel, _scrubSensitiveData.From(message)));
